...
xml字符串，作为根字符串： value有json字符串的 使用json视图展示，json视图中 可能还有 子value有json串或者xml串的情况， 需要递归支持；
按上面的2种逻辑，去优化 core下的代码逻辑。
用于测试的字符串3个， 但不要 硬编码。 实际使用的字符串和测试有变化。 3个测试字符串在 README.md文件中.
JsonStringNode和XmlStringNode应该都需要使用： looksLikeJSON, looksLikeXML判断value是否为json或者xml。 然后使用一个公共回调函数，通过
传递类型，判断是调用 LeafNode、XMLNode、JsonNode、ImageNode来展示。
